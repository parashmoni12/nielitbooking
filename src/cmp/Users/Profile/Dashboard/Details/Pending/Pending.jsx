...
import { countData,pagination,cancelBooking} from './Pending.action';
...
const Pending = ()=>{
    ...
    const {UserPendingReducer} = useSelector(response=>response);
    ...
    const filterData = () => {
        if (!UserPendingReducer || !UserPendingReducer.countSuccess) {
            return [];
        }
    
        const filteredData = UserPendingReducer.dataPag.data.filter((item) =>
    ...
    const cancelBookings = (e,id)=>{
        ...
        if (UserPendingReducer && UserPendingReducer.cancelFailed) {
    ...
    { UserPendingReducer && UserPendingReducer.countSuccess && UserPendingReducer.data!=0?
          ...
          {filterData().map((item, index) => {
                  return (
                    <TableRow key={item._id} className='animate__animated animate__fadeIn animate__slow'>
                      ...
                      <TableCell align="left"><Button variant="outlined" color="error" data-value={item._id} onClick={cancelBookings}>Delete</Button></TableCell>
                    </TableRow>
                  );
                })}
          </TableBody>
      ...
      <Pagination sx={{mt:5}} count={Math.ceil(UserPendingReducer.data/ ItemsPerPage)} page={currentPage} color="secondary" onChange={handlePageChange} />
